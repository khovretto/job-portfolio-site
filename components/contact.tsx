    ...
    <div className="surf contact-grid" style={{ padding: 28, alignItems: "center" }}>
      <div>
        ...
        <p style={{ margin: 0, color: "var(--ink-2)", fontSize: 15, maxWidth: 540 }}>
          Open to remote AI development, automation, or technical product management. Based in Novi Sad
          (GMT+2), comfortable with EU and US-East overlap.
        </p>
        <p style={{ margin: "10px 0 0", color: "var(--ink-4)", fontSize: 12.5, maxWidth: 560 }}>
          Privacy note: demo interactions may be logged for diagnostics. Raw IP addresses are not stored.
        </p>
      ...
      <div style={{ display: "flex", flexDirection: "column" }}>
        {links.map((link, index) => (
          <ContactLink key={link.key} href={link.href} targetName={link.target} className="contact-row">
            ...
            <span style={{ color: "var(--ink-4)", textAlign: "right" }}>open</span>
          </ContactLink>
        ))}
      </div>
      <style>{`
